...
public final class LumenEntityTooltipComponent implements TooltipComponent, ClientTooltipComponent {
    ...
    static Optional<TooltipComponent> create(Entity entity, LumenConfig.PreviewConfig config) {
        ...
        return entity instanceof Display && !config.displayEntities || entity instanceof ItemFrame && !config.itemFrames
                ? Optional.empty()
    ...
    public void extractImage(Font font, int x, int y, int width, int height, GuiGraphicsExtractor graphics) {
        ...
        if (state instanceof TntRenderState tntState && this.entity instanceof PrimedTnt tnt) {
            tntState.fuseRemainingInTicks = loopingFuse(tnt.getFuse(), gameTime, partialTick);
        }
        float scale;
        float centerY;
        if (state instanceof LivingEntityRenderState livingState) {
            ...
            centerY = livingState.boundingBoxHeight / 2.0F + (horse ? HORSE_CENTER_Y_OFFSET : 0.0625F);
    ...
    private void drawVisibleName(Font font, GuiGraphicsExtractor graphics, int x, int y) {
    ...
    private static void fixDisplayOrientation(DisplayEntityRenderState state, float yaw, float pitch) {
    ...
    private float scaleFor(float width, float height, float margin) {
        return margin
                * Math.min(
                        MAX_SCALE, Math.min((this.width - PADDING * 2) / width, (this.height - PADDING * 2) / height));
    }
}
