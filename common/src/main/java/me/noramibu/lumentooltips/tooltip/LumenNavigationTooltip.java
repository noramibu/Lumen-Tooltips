...
public final class LumenNavigationTooltip {
    ...
    private static final List<String> BEARINGS =
            List.of("n", "nne", "ne", "ene", "e", "ese", "se", "sse", "s", "ssw", "sw", "wsw", "w", "wnw", "nw", "nnw");
    private static final Map<MapItemSavedData, MapStats> mapStats = new WeakHashMap<>();

    private LumenNavigationTooltip() {}

    static void append(ItemStack stack, Player player, List<Component> tooltip, LumenConfig.NavigationConfig config) {
        if (config.maps) {
    ...
    private static void appendMap(ItemStack stack, List<Component> tooltip) {
        ...
        if (stats.outsideDirection() != null) {
            tooltip.add(line("map_outside", directionComponent(stats.outsideDirection())));
        }
        if (stats.nearestUnexploredDirection() != null) {
            tooltip.add(line("map_nearest_unexplored", directionComponent(stats.nearestUnexploredDirection())));
        }
    }
    ...
    private static MapStats analyzeMap(MapItemSavedData data) {
        ...
        String outside = playerMarker != null && isOutsideMarker(playerMarker)
                ? mapDirection(playerMarker.x(), playerMarker.y())
                : null;
        return analyzePixels(data.scale, data.colors, sourceX, sourceZ, outside);
    }

    private static MapStats analyzePixels(byte scale, byte[] colors, int sourceX, int sourceZ, String outside) {
        int explored = 0;
        ...
        String unexplored = nearestDistance == Integer.MAX_VALUE ? null : mapDirection(nearestX, nearestZ);
        return new MapStats(scale, explored * 100 / colors.length, unexplored, outside);
    }
    ...
    private static boolean isPlayerMarker(MapDecoration decoration) {
        return decoration.type().equals(MapDecorationTypes.PLAYER) || isOutsideMarker(decoration);
    }
    ...
    private static GlobalPos target(ItemStack stack, Player player) {
        ...
        return stack.is(Items.RECOVERY_COMPASS) ? player.getLastDeathLocation().orElse(null) : null;
    }
    ...
    private static void appendTarget(GlobalPos target, Player player, List<Component> tooltip) {
        ...
        if (distance > 0) {
            tooltip.add(line(
                    "bearing", Component.translatable("tooltip.lumen_tooltips.navigation.bearing." + bearing(dx, dz))));
        }
    }
    ...
    private static String direction(double dx, double dz, List<String> directions) {
        int index = Math.floorMod(
                (int) Math.round(Math.atan2(dx, -dz) * directions.size() / (Math.PI * 2.0)), directions.size());
        return directions.get(index);
    }

    private static Component directionComponent(String direction) {
        return Component.translatable("tooltip.lumen_tooltips.navigation.direction." + direction);
    }

    private static Component line(String key, Object... arguments) {
        return Component.translatable("tooltip.lumen_tooltips.navigation." + key, arguments)
                .withStyle(ChatFormatting.GRAY);
    }

    private record MapStats(
            byte scale, int exploredPercent, String nearestUnexploredDirection, String outsideDirection) {}
}
