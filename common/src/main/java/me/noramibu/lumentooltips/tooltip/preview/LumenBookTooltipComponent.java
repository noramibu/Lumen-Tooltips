...
record LumenBookTooltipComponent(Component page, LumenConfig.PreviewConfig config, Identifier style)
    ...
    static Optional<TooltipComponent> create(ItemStack stack) {
        ...
        return pages.isEmpty()
                ? Optional.empty()
                : create(pages.getFirst(), pages.size(), stack, LumenConfigManager.current().modules.preview);
    }

    static Optional<TooltipComponent> create(Component page, int pageCount) {
        return create(page, pageCount, ItemStack.EMPTY, LumenConfigManager.current().modules.preview);
    }

    private static Optional<TooltipComponent> create(
            Component page, int pageCount, ItemStack stack, LumenConfig.PreviewConfig config) {
        return Optional.of(new LumenBookTooltipComponent(
                page.copy()
                        .append(" ")
                        .append(Component.translatable("tooltip.lumen_tooltips.book.pages", pageCount)
    ...
    public void extractImage(Font font, int x, int y, int width, int height, GuiGraphicsExtractor graphics) {
        LumenPreviewStyle.drawPanel(graphics, x, y, getWidth(font), getHeight(font), this.style);
        Matrix3x2fStack pose = graphics.pose();
        pose.pushMatrix();
        pose.translate(x, y);
        pose.scale(scale(), scale());
        graphics.blit(RenderPipelines.GUI_TEXTURED, BACKGROUND, 0, 0, 0, 0, 128, 128, 179, 179);
