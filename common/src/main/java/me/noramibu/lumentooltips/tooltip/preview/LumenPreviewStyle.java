...
final class LumenPreviewStyle {
    private static final Identifier BACKGROUND = Identifier.withDefaultNamespace("tooltip/background");
    ...
    static void drawPanel(
            GuiGraphicsExtractor graphics, int x, int y, int width, int height, @Nullable Identifier style) {
        graphics.blitSprite(
                RenderPipelines.GUI_TEXTURED, sprite(style, "_background", BACKGROUND), x, y, width, height);
        graphics.blitSprite(RenderPipelines.GUI_TEXTURED, sprite(style, "_frame", FRAME), x, y, width, height);
    }
    ...
    private static Identifier sprite(@Nullable Identifier style, String suffix, Identifier fallback) {
        return style == null
                ? fallback
                : Identifier.fromNamespaceAndPath(style.getNamespace(), "tooltip/" + style.getPath() + suffix);
    }
}
