...
final class LumenAreaEffectCloudTooltipComponent implements TooltipComponent, ClientTooltipComponent {
    ...
    LumenAreaEffectCloudTooltipComponent(AreaEffectCloud cloud, LumenConfig.PreviewConfig config) {
        this.cloud = cloud;
        this.config = config;
    }
    ...
    public void extractImage(Font font, int x, int y, int width, int height, GuiGraphicsExtractor graphics) {
        ...
        int color = options instanceof ColorParticleOption colored
                ? ARGB.colorFromFloat(colored.getAlpha(), colored.getRed(), colored.getGreen(), colored.getBlue())
                : -1;
        ...
        for (int index = 0; index < particleCount; index++) {
            ...
            int particleY = centerY + (int) (positions[offset + 1] * particleRadiusY) - (int) (age * 6.0);
            int particleSize = positions[offset + 2] > 0.65F ? 6 : 5;
            int renderedColor =
                    this.cloud.isWaiting() && options.getType() == ParticleTypes.ENTITY_EFFECT && index % 2 == 0
                            ? -1
                            : color;
            int particleColor = ARGB.multiplyAlpha(renderedColor, (float) ((1.0 - age) * 0.375 + 0.375));
            if (sprite == null) {
    ...
    private TextureAtlasSprite particleSprite(Minecraft minecraft) {
        if (!this.particleResolved && minecraft.level != null) {
            ...
            Particle particle = ((ParticleEngineAccessor) minecraft.particleEngine)
    ...
    private static void fillEllipse(GuiGraphicsExtractor graphics, int centerX, int centerY, int radiusX, int radiusY) {
        for (int offsetY = -radiusY; offsetY <= radiusY; offsetY++) {
            int span = ellipseSpan(radiusX, radiusY, offsetY);
            graphics.fill(centerX - span, centerY + offsetY, centerX + span + 1, centerY + offsetY + 1, 0x20000000);
        }
    }
    ...
    private static int particleCount(float radius, boolean waiting) {
        return waiting ? 2 : Math.clamp((int) Math.ceil(Math.PI * radius * radius), 1, MAX_PARTICLES);
    }
