...
record LumenFireworkTooltipComponent(
        int flightDuration, List<FireworkExplosion> explosions, LumenConfig.PreviewConfig config, Identifier style)
    ...
    LumenFireworkTooltipComponent(int flightDuration, List<FireworkExplosion> explosions) {
        this(flightDuration, explosions, LumenConfigManager.current().modules.preview, null);
    }

    static Optional<TooltipComponent> create(ItemStack stack, LumenConfig.PreviewConfig config) {
        Fireworks fireworks = stack.get(DataComponents.FIREWORKS);
        if (fireworks != null) {
            return Optional.of(new LumenFireworkTooltipComponent(
        ...
        return explosion == null
                ? Optional.empty()
                : Optional.of(new LumenFireworkTooltipComponent(
    ...
    public void extractImage(Font font, int x, int y, int width, int height, GuiGraphicsExtractor graphics) {
        LumenPreviewStyle.drawPanel(graphics, x, y, panelWidth(), getHeight(font), this.style);
        Component header = this.flightDuration >= 0
    ...
    private void drawSimulation(GuiGraphicsExtractor graphics, int centerX, int top) {
        ...
        float time = this.config.reducedMotion
        ...
        int explosionTicks =
                this.explosions.isEmpty() ? 0 : (this.explosions.size() - 1) * EXPLOSION_INTERVAL + MAX_SPARK_LIFETIME;
        ...
        for (int index = 0; index < this.explosions.size(); index++) {
            ...
            if (age >= 0.0F && age < MAX_SPARK_LIFETIME) {
                drawExplosion(graphics, this.explosions.get(index), centerX, centerY, age, index, burstLift);
            }
        }
    }

    private static void drawRocket(
            GuiGraphicsExtractor graphics, int centerX, int endY, int startY, float age, int lifetime) {
    ...
    private void drawExplosion(
        ...
        if (age < 4.0F) {
            ...
            graphics.fill(
                    ...
                    ARGB.color(
                            flashAlpha,
                            explosion.colors().isEmpty()
                                    ? 0x1D1D21
                                    : explosion.colors().getInt(0)));
        }
        SparkFrame frame = new SparkFrame(
                age, explosionIndex, dampedDistance(age), gravityDrop(age), (float) Math.pow(0.8, age - (int) age));
        switch (explosion.shape()) {
            case SMALL_BALL -> drawBall(graphics, explosion, centerX, centerY, frame, SMALL_BALL_VELOCITIES);
            case LARGE_BALL -> drawBall(graphics, explosion, centerX, centerY, frame, LARGE_BALL_VELOCITIES);
            case STAR -> drawShape(graphics, explosion, centerX, centerY, frame, STAR_POINTS, false);
            case CREEPER -> drawShape(graphics, explosion, centerX, centerY, frame, CREEPER_POINTS, true);
            case BURST -> drawBurst(graphics, explosion, centerX, centerY, frame, burstLift);
        }
    }
        ...
        for (int angleStep = 0; angleStep < 3; angleStep++) {
            ...
            for (int segment = 1; segment < points.length; segment++) {
                for (int step = 1; step <= 4; step++) {
                    ...
                    double velocityX = Mth.lerp(amount, points[segment - 1][0], points[segment][0]) * 0.5;
                    double velocityY = Mth.lerp(amount, points[segment - 1][1], points[segment][1]) * 0.5;
            ...
            for (int trail = 0; trail < 4 && spawnAge > 0; trail++, spawnAge -= 2) {
                ...
                if (trailAge < trailLifetime && visible(explosion.hasTwinkle(), trailAge, trailLifetime)) {
                    drawSparkPoint(
                            ...
                            scale,
                            ARGB.multiplyAlpha(
                                    sparkColor(explosion, seed, trailAge, trailLifetime, frame.fadePartial), 0.7F),
                            1);
                }
            }
        }
    ...
    private static void drawSparkPoint(
            ...
            int color,
            int size) {
        int x = centerX + Math.round((float) ((velocityX + velocityZ * 0.35) * distance * scale));
        int y = centerY - Math.round((float) (velocityY * distance * scale)) + Math.round(drop * scale);
        graphics.fill(x, y, x + size, y + size, color);
    }

    private static int sparkColor(FireworkExplosion explosion, int seed, float age, int lifetime, float fadePartial) {
        IntList colors = explosion.colors();
        int color =
                colors.isEmpty() ? 0x1D1D21 : colors.getInt(Math.floorMod(Mth.murmurHash3Mixer(seed), colors.size()));
        ...
        if (!fades.isEmpty() && age > halfLife) {
            ...
            color = ARGB.srgbLerp(
                    fade, color, fades.getInt(Math.floorMod(Mth.murmurHash3Mixer(seed + 1), fades.size())));
        }
    ...
    private static boolean visible(boolean twinkle, float age, int lifetime) {
        return !twinkle || age < lifetime / 3.0F || ((int) age + lifetime) / 3 % 2 == 0;
    }

    private static float sparkAlpha(float age, int lifetime) {
        return age > lifetime / 2.0F ? 1.0F - (age - lifetime / 2.0F) / lifetime : 0.99F;
    }

    private static int sparkLifetime(int seed) {
        return MIN_SPARK_LIFETIME + Math.floorMod(Mth.murmurHash3Mixer(seed), SPARK_LIFETIME_RANGE);
    }
    ...
    private static float rocketHeight(float age) {
        int ticks = (int) age;
        float partial = age - ticks;
        return 0.05F * ticks + 0.02F * ticks * (ticks + 1) + partial * (0.05F + 0.04F * (ticks + 1));
    }
            ...
            for (int xStep = -steps; xStep <= steps; xStep++) {
                for (int zStep = -steps; zStep <= steps; zStep++) {
                    ...
                    if (index % stride == 0) {
                        ...
                        double length = Math.sqrt(x * x + y * y + z * z) / speed + gaussian(seed + 6) * 0.05;
    ...
    private static double[][] burstVelocities() {
        ...
        for (int index = 0; index < velocities.length; index++) {
            ...
            velocities[index] = new double[] {
                gaussian(seed) * 0.15 + offsetX, random(seed + 2) * 0.5, gaussian(seed + 4) * 0.15 + offsetZ
            };
        }
        return velocities;
    }
    ...
    private static double gaussian(int seed) {
        return Math.sqrt(-2.0 * Math.log(Math.max(0.000001, random(seed)))) * Math.cos(Math.TAU * random(seed + 1));
    }
    ...
    private void drawExplosion(Font font, GuiGraphicsExtractor graphics, FireworkExplosion explosion, int x, int y) {
    ...
    private static int drawSwatches(GuiGraphicsExtractor graphics, IntList colors, int x, int y, int maxX) {
        for (int index = 0; index < colors.size() && x + SWATCH_SIZE <= maxX; index++) {
            graphics.fill(x, y, x + SWATCH_SIZE, y + SWATCH_SIZE, 0xFF000000 | colors.getInt(index));
    ...
    private record SparkFrame(float age, int explosionIndex, float distance, float drop, float fadePartial) {}
}
