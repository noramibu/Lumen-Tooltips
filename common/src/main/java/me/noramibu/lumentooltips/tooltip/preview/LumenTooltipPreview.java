...
public final class LumenTooltipPreview {
    ...
    public static Optional<TooltipComponent> create(ItemStack stack) {
        ...
        if (!preview.enabled || !preview.activation.isActive(currentFlag(), preview.key)) {
    ...
    private static Optional<TooltipComponent> createUncached(ItemStack stack, LumenConfig.PreviewConfig preview) {
        return (preview.itemDetails ? createItemDetails(stack, preview) : Optional.<TooltipComponent>empty())
                .or(() -> preview.maps ? LumenMapTooltipComponent.create(stack, preview) : Optional.empty())
                .or(() -> createVisualItemPreview(stack, preview))
                .or(() -> preview.potions ? LumenPotionTooltipComponent.create(stack, preview) : Optional.empty())
                .or(() -> preview.signs ? createSignPreview(stack, preview) : Optional.empty())
                .or(() -> createEnderChestPreview(stack, preview))
                .or(() -> LumenEntityPreviewResolver.create(stack, preview)
    ...
    public static Optional<TooltipComponent> configSample(String path, LumenConfig config) {
        ...
        return switch (path) {
            ...
            case "modules.preview.playerHeads" ->
                Optional.of(new LumenItemVisualTooltipComponent(new ItemStack(Items.PLAYER_HEAD), preview));
            ...
            case "modules.preview.density", "modules.preview.fireworks", "modules.preview.reducedMotion" ->
    ...
    private static Optional<TooltipComponent> createVisualItemPreview(
        ...
        return config.paintings && stack.is(Items.PAINTING) && !hasPaintingEntity(stack)
                ? Optional.of(new LumenItemVisualTooltipComponent(stack, config))
                : Optional.empty();
    }

    private static Optional<TooltipComponent> createSignPreview(ItemStack stack, LumenConfig.PreviewConfig config) {
    ...
    private static boolean hasPaintingEntity(ItemStack stack) {
        ...
        return data != null && data.type() == EntityTypes.PAINTING;
    }

    private static boolean supportsVisualItem(ItemStack stack, LumenConfig.PreviewConfig config) {
        return config.banners && (stack.getItem() instanceof BannerItem || stack.is(Items.SHIELD))
                || config.decoratedPots && stack.is(Items.DECORATED_POT)
                || config.playerHeads && stack.is(Items.PLAYER_HEAD)
                || config.paintings && stack.is(Items.PAINTING) && !hasPaintingEntity(stack);
    }

    private static Optional<TooltipComponent> createBundlePreview(ItemStack stack, LumenConfig.PreviewConfig config) {
        if (!config.bundles) {
            return Optional.empty();
        }
    ...
    private static boolean supportsItemDetails(ItemStack stack, LumenConfig.PreviewConfig config) {
        return config.books
    ...
    private static Optional<TooltipComponent> createItemDetails(ItemStack stack, LumenConfig.PreviewConfig config) {
        return (config.books ? LumenBookTooltipComponent.create(stack) : Optional.<TooltipComponent>empty())
                .or(() -> config.fireworks ? LumenFireworkTooltipComponent.create(stack, config) : Optional.empty());
    }
    ...
    private static TooltipComponent containerPreview(
            ItemStack source, List<ItemStack> stored, boolean shulker, LumenConfig.PreviewConfig config) {
    ...
    private static TooltipComponent containerPreview(
        ...
        int columns = config.containerMode == ContainerPreviewMode.COMPACT
                ...
                : CONTAINER_COLUMNS;
        int rows = config.containerMode == ContainerPreviewMode.COMPACT
