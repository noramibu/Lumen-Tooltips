...
public final class LumenFoodTooltipComponent implements Component, FormattedCharSequence, ClientTooltipComponent {
    ...
    public LumenFoodTooltipComponent(
            int hunger, float saturation, boolean rotten, boolean showHunger, boolean showSaturation) {
        this.hunger = hunger;
        ...
        this.saturationText = saturationIcons > 10 || saturationIcons == 0
    ...
    public int getWidth(Font font) {
        int hungerWidth = this.showHunger ? this.hungerIcons * 9 + textWidth(font, this.hungerText) : 0;
        int saturationWidth = this.showSaturation ? this.saturationIcons * 7 + textWidth(font, this.saturationText) : 0;
        return hungerWidth + saturationWidth + (this.showHunger && this.showSaturation ? 2 : 0);
    }
    ...
    public void extractImage(Font font, int x, int y, int width, int height, GuiGraphicsExtractor graphics) {
        ...
        if (this.showSaturation) {
            int hungerWidth = this.showHunger ? this.hungerIcons * 9 + textWidth(font, this.hungerText) + 2 : 0;
    ...
    private void drawHunger(Font font, GuiGraphicsExtractor graphics, int startX, int y) {
        ...
        for (int value = 0; value < this.hungerIcons * 2; value += 2) {
            graphics.blitSprite(RenderPipelines.GUI_TEXTURED, this.rotten ? FOOD_EMPTY_HUNGER : FOOD_EMPTY, x, y, 9, 9);
            if (hunger > value) {
                boolean half = hunger - 1 == value;
                graphics.blitSprite(
                        RenderPipelines.GUI_TEXTURED,
                        this.rotten ? half ? FOOD_HALF_HUNGER : FOOD_FULL_HUNGER : half ? FOOD_HALF : FOOD_FULL,
                        x,
                        y,
                        9,
                        9);
    ...
    private static void drawSmallText(Font font, GuiGraphicsExtractor graphics, String text, int x, int y) {
        if (text == null) {
            return;
        }
