...
public final class LumenBundleTooltipComponent implements TooltipComponent, ClientTooltipComponent {
    ...
    public void extractImage(Font font, int x, int y, int width, int height, GuiGraphicsExtractor graphics) {
        for (int index = 0; index < this.items.size(); index++) {
            ...
            graphics.blitSprite(
                    RenderPipelines.GUI_TEXTURED, selected ? SELECTED_BACK : SLOT, slotX, slotY, SLOT_SIZE, SLOT_SIZE);
            int itemOffset = 4;
            ...
            if (selected) {
                graphics.blitSprite(RenderPipelines.GUI_TEXTURED, SELECTED_FRONT, slotX, slotY, SLOT_SIZE, SLOT_SIZE);
            }
        }
        drawSelectedItemTooltip(font, graphics, x, y, width);
        drawProgressBar(font, graphics, x + (getWidth(font) - BAR_WIDTH) / 2, y + getHeight(font) - BAR_HEIGHT - 4);
    }
    ...
    private void drawProgressBar(Font font, GuiGraphicsExtractor graphics, int x, int y) {
        ...
        graphics.blitSprite(
                RenderPipelines.GUI_TEXTURED, fullness >= 1.0F ? BAR_FULL : BAR_FILL, x + 1, y, fillWidth, BAR_HEIGHT);
        graphics.blitSprite(RenderPipelines.GUI_TEXTURED, BAR_BORDER, x, y, BAR_WIDTH, BAR_HEIGHT);
        Component label = fullness >= 1.0F ? BUNDLE_FULL : Component.literal(Math.round(fullness * 100.0F) + "%");
