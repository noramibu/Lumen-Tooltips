...
final class LumenExtraStatistics {
    ...
    static void append(
            ItemStack stack, List<Component> tooltip, TooltipFlag flag, LumenConfig.ExtraStatisticsConfig config) {
        ...
        if (config.useCooldown) {
            ...
            if (cooldown != null) {
                add(tooltip, "use_cooldown", duration(cooldown.ticks(), cooldown.seconds(), config.useSeconds));
            }
        }
    ...
    private static void appendBlockStatistics(
            ItemStack stack, List<Component> tooltip, LumenConfig.ExtraStatisticsConfig config) {
        ...
        if (config.enchantmentPower && block.defaultBlockState().is(BlockTags.ENCHANTMENT_POWER_PROVIDER)) {
    ...
    private static Optional<Component> miningLevel(Tool tool) {
        for (Tool.Rule rule : tool.rules()) {
            if (!rule.correctForDrops().orElse(true)) {
                Optional<Component> level = rule.blocks()
                        .unwrapKey()
    ...
    private static void addPositive(List<Component> tooltip, String key, float value, String suffix) {
        if (value > 0.0F) {
    ...
    private static void add(List<Component> tooltip, String key, Component value) {
        tooltip.add(Component.translatable("tooltip.lumen_tooltips.extra." + key, value)
                .withStyle(ChatFormatting.DARK_GRAY));
    }

    private static Component duration(int ticks, float seconds, boolean useSeconds) {
        return Component.translatable(
                useSeconds ? "tooltip.lumen_tooltips.extra.seconds" : "tooltip.lumen_tooltips.extra.ticks",
    ...
    private static Component miningLevelName(String level) {
        ...
        return Language.getInstance().has(key) ? Component.translatable(key) : Component.literal(humanize(level));
    }

    private static String humanize(String value) {
        String words = value.replace('_', ' ');
        return words.isEmpty() ? value : Character.toUpperCase(words.charAt(0)) + words.substring(1);
    }
}
