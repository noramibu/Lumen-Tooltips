...
final class LumenMapTooltipComponent implements TooltipComponent, ClientTooltipComponent {
    ...
    static Optional<TooltipComponent> create(ItemStack stack, LumenConfig.PreviewConfig config) {
        ...
        MapItemSavedData data = id == null || minecraft.level == null ? null : minecraft.level.getMapData(id);
        return data == null
                ? Optional.empty()
                : Optional.of(new LumenMapTooltipComponent(id, data, config, stack.get(DataComponents.TOOLTIP_STYLE)));
    }
    ...
    public void extractImage(Font font, int x, int y, int width, int height, GuiGraphicsExtractor graphics) {
        ...
        int size = size();
        graphics.blit(
                RenderPipelines.GUI_TEXTURED, MAP_BACKGROUND, x + padding, y + padding, 0, 0, size, size, size, size);
        float scale = size / 128.0F;
        graphics.pose().pushMatrix();
        graphics.pose().translate(x + padding, y + padding);
        graphics.pose().scale(scale, scale);
        Minecraft.getInstance().getMapRenderer().extractRenderState(this.id, this.data, this.renderState);
