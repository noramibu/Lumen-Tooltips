...
public final class LumenTooltipAppender {
    ...
    public static void append(
            Item.TooltipContext context, ItemStack stack, Player player, List<Component> tooltip, TooltipFlag flag) {
    ...
    private static void suppressPotionText(Item.TooltipContext context, ItemStack stack, List<Component> tooltip) {
    ...
    private static void appendEnderChestNotice(
            ItemStack stack, List<Component> tooltip, TooltipFlag flag, LumenConfig.PreviewConfig config) {
        if (config.enabled
                && config.enderChest
                && stack.is(Items.ENDER_CHEST)
                && config.activation.isActive(flag, config.key)
                && !LumenEnderChestMemory.isKnown()) {
            tooltip.add(Component.translatable("tooltip.lumen_tooltips.ender_chest_unknown")
    ...
    private static void appendControlHints(ItemStack stack, List<Component> tooltip, LumenConfig config) {
        LumenConfig.PreviewConfig preview = config.modules.preview;
        ...
        if (hints.showPreviewHint
            ...
            tooltip.add(controlHint(preview.key, "tooltip.lumen_tooltips.action.preview"));
        }
        String openAction =
                hints.showOpenContainerHint && preview.openContainers && LumenContainerOpener.isContainer(stack)
                        ? "tooltip.lumen_tooltips.action.open_container"
                        : hints.showOpenBookHint && preview.openBooks && LumenContainerOpener.isBook(stack)
    ...
    public static Component controlHint(String key, String actionKey) {
        ...
        Component keyName = LumenInputBinding.displayName(key).setStyle(Style.EMPTY.withColor(ChatFormatting.YELLOW));
    ...
    private static void enhanceDurability(int maxDamage, int damage, List<Component> tooltip, LumenConfig config) {
        if ((!config.modules.durability.showPercent && !config.modules.durability.useColors) || maxDamage <= 0) {
            return;
        }
    ...
    private static void appendFood(
        ...
        if (config.modules.food.showHunger || config.modules.food.showSaturation) {
            ...
            tooltip.add(new LumenFoodTooltipComponent(
                    food.nutrition(),
                    food.saturation(),
                    isRotten(consumable),
    ...
    private static void appendEnchantments(
        ...
        Set<Holder<Enchantment>> enchantments =
                new LinkedHashSet<>(stack.getOrDefault(DataComponents.ENCHANTMENTS, ItemEnchantments.EMPTY)
                        .keySet());
        enchantments.addAll(stack.getOrDefault(DataComponents.STORED_ENCHANTMENTS, ItemEnchantments.EMPTY)
                .keySet());
        for (Holder<Enchantment> enchantment : enchantments) {
            enchantment
                    ...
                    .ifPresent(descriptionKey -> tooltip.add(Component.translatable(
