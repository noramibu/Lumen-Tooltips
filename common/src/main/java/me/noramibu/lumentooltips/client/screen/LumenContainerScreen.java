...
public final class LumenContainerScreen extends ContainerScreen {
    ...
    LumenContainerScreen(
            ItemStack containerItem, List<ItemStack> containerItems, Inventory playerInventory, Screen parent) {
        super(createMenu(containerItems, playerInventory), playerInventory, containerItem.getHoverName());
        this.parent = parent;
        this.containerItem = containerItem.copy();
        this.breadcrumb = parent instanceof LumenContainerScreen container
    ...
    protected void init() {
        ...
        if (this.parent instanceof LumenContainerScreen) {
            this.backButton = this.addRenderableWidget(new PageButton(
                    this.leftPos + this.imageWidth - 29, this.topPos + 5, false, button -> onClose(), false));
            this.backButton.setTooltip(Tooltip.create(Component.translatable("screen.lumen_tooltips.container.back")));
        }
    }

    @Override
    protected void extractLabels(GuiGraphicsExtractor graphics, int mouseX, int mouseY) {
        Component label = this.parent instanceof LumenContainerScreen
    ...
    private Component breadcrumbLabel(int maxWidth) {
        ...
        return Component.literal(prefix
                + this.font.plainSubstrByWidth(this.breadcrumb.getString(), maxWidth - this.font.width(prefix), true));
    }
    ...
    public boolean keyPressed(KeyEvent event) {
        if (this.hoveredSlot != null) {
            ...
            if (LumenItemEditor.trySaveToStorage(stack, event) || LumenContainerOpener.tryOpen(stack, event)) {
                return true;
            }
        }
