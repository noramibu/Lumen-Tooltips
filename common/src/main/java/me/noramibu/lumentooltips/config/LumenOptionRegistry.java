...
public final class LumenOptionRegistry {
    ...
    private static final List<ConfigOption> ACTIVE_OPTIONS = Stream.concat(OPTIONS.stream(), tooltipFlagOptions())
            .filter(option -> LumenItemEditor.isAvailable() || !ITEM_EDITOR_KEY_PATH.equals(option.path()))
            .filter(option -> LumenItemEditor.isAvailable() || !option.path().startsWith(ITEM_EDITOR_STORAGE_PREFIX))
            .toList();
    ...
    private static ConfigOption hiddenComponentOption(String id) {
        return ConfigOption.hiddenComponent(
                id, config -> config.modules.tooltip.ignoredHiddenComponents.contains(id), (config, value) -> {
    ...
    public static boolean isVisible(ConfigOption option, LumenConfig config) {
        ...
        return switch (option.path()) {
            ...
            case "modules.navigation.maps", "modules.navigation.compasses" -> config.modules.navigation.enabled;
            case "modules.extraStatistics.key" -> config.modules.extraStatistics.activation == HoldMode.KEY;
            case "modules.tooltip.scrollStep" -> config.modules.tooltip.scrollLongTooltips;
            case "modules.tooltip.showPreviewHint" ->
                config.modules.tooltip.showControlHints && preview.enabled && preview.activation == HoldMode.KEY;
            case "modules.tooltip.showOpenContainerHint" ->
                config.modules.tooltip.showControlHints && preview.enabled && preview.openContainers;
            ...
            case "modules.preview.key" -> preview.enabled && preview.activation == HoldMode.KEY;
            ...
            case "modules.preview.containerTintPercent" ->
                preview.enabled && preview.accents && (preview.shulkers || preview.containers || preview.enderChest);
            case "modules.preview.books", "modules.preview.fireworks" -> preview.enabled && preview.itemDetails;
