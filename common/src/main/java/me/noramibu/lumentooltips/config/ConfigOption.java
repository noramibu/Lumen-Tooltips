...
public record ConfigOption(
    ...
    private static final String HIDDEN_COMPONENT_KEY = "config.lumen_tooltips.modules.tooltip_flags.hidden_component";
    ...
    public static ConfigOption toggle(
            String path, Function<LumenConfig, Boolean> getter, BiConsumer<LumenConfig, Boolean> setter) {
    ...
    public static ConfigOption hiddenComponent(
            String id, Function<LumenConfig, Boolean> getter, BiConsumer<LumenConfig, Boolean> setter) {
    ...
    public static ConfigOption integer(
        ...
        return numeric(path, OptionControl.INTEGER, minValue, maxValue, step, getter, setter);
    }
    ...
    public static ConfigOption percent(
            ...
            Function<LumenConfig, Integer> getter,
            BiConsumer<LumenConfig, Integer> setter) {
        return numeric(path, OptionControl.PERCENT, minValue, maxValue, step, getter, setter);
    }
    ...
    private static ConfigOption numeric(
        ...
        return create(
                ...
                (config, value) -> setter.accept(config, Math.clamp(Integer.parseInt(value), minValue, maxValue)),
                minValue,
                maxValue,
                step,
                List.of());
    ...
    public static ConfigOption keyBind(
            String path, Function<LumenConfig, String> getter, BiConsumer<LumenConfig, String> setter) {
    ...
    public static ConfigOption text(
            String path, int maxLength, Function<LumenConfig, String> getter, BiConsumer<LumenConfig, String> setter) {
        String defaultValue = getter.apply(DEFAULTS);
        return create(
                path,
                OptionControl.TEXT,
                defaultValue,
                getter,
                (config, value) -> setter.accept(config, value.substring(0, Math.min(value.length(), maxLength))),
                0,
                maxLength,
                1,
                List.of());
    ...
    private static <E extends Enum<E>> ConfigOption enumCycle(
            String path, Function<LumenConfig, E> getter, BiConsumer<LumenConfig, E> setter, List<String> cycleValues) {
    ...
    private static ConfigOption create(
        ...
        return new ConfigOption(
                path, languageKey(path), control, defaultValue, getter, setter, minValue, maxValue, step, cycleValues);
    }
    ...
    public Component valueComponent(LumenConfig config) {
        ...
        return switch (this.control) {
            ...
            case NUMBER, TEXT ->
                CommonComponents.optionNameValue(title, colored(Component.literal(value), ChatFormatting.AQUA));
            case PERCENT ->
                CommonComponents.optionNameValue(title, colored(Component.literal(value + "%"), ChatFormatting.AQUA));
            case CYCLE ->
                        ...
                        colored(
                                switch (this.path) {
                                    ...
                                    case "modules.durability.palette" ->
                                        enumValue(value, DurabilityPalette.DEFAULT)
                                                .displayName();
            ...
            case KEY_BIND ->
                CommonComponents.optionNameValue(
                        ...
                        colored(
                                ...
                                LumenInputBinding.UNBOUND.equals(value) ? ChatFormatting.RED : ChatFormatting.YELLOW));
        };
    }
    ...
    private static Component enchantmentPreview(LumenConfig config) {
        ...
        Component level = config.modules.enchantments.decimalLevels
                ? Component.literal("5")
                : Component.translatable("enchantment.level.5");
        Component enchantment = Component.translatable("enchantment.minecraft.sharpness")
                .append(" ")
                .append(level)
    ...
    private static Component foodPreview(LumenConfig config) {
        ...
        if (config.modules.food.showHunger) {
            preview.append("\n")
                    .append(Component.translatable("screen.lumen_tooltips.config.preview.hunger", 6)
                            .withStyle(ChatFormatting.RED));
        }
        if (config.modules.food.showSaturation) {
            preview.append("\n")
                    .append(Component.translatable("screen.lumen_tooltips.config.preview.saturation", "7.2")
                            .withStyle(ChatFormatting.GOLD));
        }
        if (config.modules.food.showEffects) {
            preview.append("\n")
                    .append(Component.translatable("effect.minecraft.night_vision")
                            .append(" (0:05)")
    ...
    private static Component navigationPreview(LumenConfig config, boolean compass) {
        MutableComponent preview =
                Component.translatable(compass ? "item.minecraft.compass" : "item.minecraft.filled_map");
    ...
    public void cycle(LumenConfig config) {
        switch (this.control) {
            case TOGGLE -> this.setter.accept(config, Boolean.toString(!Boolean.parseBoolean(getAsString(config))));
